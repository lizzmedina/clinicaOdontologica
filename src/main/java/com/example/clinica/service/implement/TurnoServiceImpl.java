...
public class TurnoServiceImpl implements ITurnoService<TurnoDto> {
   ...
   @Override
   public TurnoDto crearTurno(int idPaciente, int idOdontologo, LocalDate fecha) throws Exception {
       Turno turno = null;
       TurnoDto turnoDto;
       if (!fecha.isBefore(LocalDate.now())) {
           try {
               PacienteDto pacienteDto = pacienteService.obtenerPorId(idPaciente);
               turno.setPaciente(mapper.getModelMapper().map(pacienteDto, Paciente.class));
           } catch (ServiceException e) {
               throw new Exception(e.getMessage());
           }
           try {
               OdontologoDto odontologoDto = odontologoService.obtenerPorId(idOdontologo);
